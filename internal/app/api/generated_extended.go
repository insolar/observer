...
type ResponsesAugmentedAddressYaml struct {
...
type ResponsesInvalidSignatureYaml struct {
...
type ServerInterface interface {
	...
	// Member ethereum augmented address
	// (GET /api/member/augmentedAddress/{reference})
	AugmentedAddress(ctx echo.Context, reference string) error
	// Member ethereum augmented address
	// (POST /api/member/augmentedAddress/{reference})
	SetAugmentedAddress(ctx echo.Context, reference string, params SetAugmentedAddressParams) error
...
func (w *ServerInterfaceWrapper) AugmentedAddress(ctx echo.Context) error {
	...
	// Invoke the callback with all the unmarshalled arguments
	err = w.Handler.AugmentedAddress(ctx, reference)
	return err
}
...
func (w *ServerInterfaceWrapper) SetAugmentedAddress(ctx echo.Context) error {
	...
	var params SetAugmentedAddressParams

	headers := ctx.Request().Header
	// ------------- Required header parameter "Digest" -------------
	if valueList, found := headers[http.CanonicalHeaderKey("Digest")]; found {
		var Digest DigestHeader
		n := len(valueList)
		if n != 1 {
			return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("Expected one value for Digest, got %d", n))
		}

		err = runtime.BindStyledParameter("simple", false, "Digest", valueList[0], &Digest)
		if err != nil {
		...
		return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("Header parameter Digest is required, but not found"))
	}
	...
	if valueList, found := headers[http.CanonicalHeaderKey("Signature")]; found {
		...
		n := len(valueList)
		if n != 1 {
			return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("Expected one value for Signature, got %d", n))
		}

		err = runtime.BindStyledParameter("simple", false, "Signature", valueList[0], &Signature)
		if err != nil {
		...
		return echo.NewHTTPError(http.StatusBadRequest, fmt.Sprintf("Header parameter Signature is required, but not found"))
	}

	// Invoke the callback with all the unmarshalled arguments
	err = w.Handler.SetAugmentedAddress(ctx, reference, params)
	return err
}
...
func RegisterHandlers(router EchoRouter, si ServerInterface) {
	...
	router.GET("/api/member/augmentedAddress/:reference", wrapper.AugmentedAddress)
	router.POST("/api/member/augmentedAddress/:reference", wrapper.SetAugmentedAddress)
